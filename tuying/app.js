...
(() => {
  ...
  function library() {
    ...
    return head(state.page==='albums'?'图包管理':'数据包管理',btn('商品视图','product-view','','','grid')+(state.page==='albums'?btn('生成图包','make-album','primary','','plus'):btn('添加数据包','import','primary','','plus')),`${packs.length} 个${state.page==='albums'?'图包':'数据包'}`)+
      `<section class="pk-filters"><div class="pk-filter-fields"><div class="segmented pk-type">${[['all','全部'],['数据包','数据包'],['图包','图包']].map(([key,label])=>`<button class="segment ${state.packageType===key?'active':''}" data-action="package-type" data-value="${key}">${label}</button>`).join('')}</div><label class="pk-field"><span>数据包名称</span><input id="packageSearch" value="${e(state.packageQuery)}" placeholder="搜索数据包"></label><label class="pk-field"><span>所属分组</span><select id="packageGroup"><option value="all">全部</option>${['未分组',...state.groups].map(g=>`<option value="${e(g)}" ${state.packageGroup===g?'selected':''}>${e(g)}</option>`).join('')}</select></label>${btn('搜索','package-search','primary','','search')}${btn('重置','package-reset','','','refresh')}</div><div class="pk-management${state.selectedPacks.size?' is-expanded':''}"><button type="button" class="pk-management-toggle" data-action="toggle-package-tools" aria-expanded="${Boolean(state.selectedPacks.size)}">管理操作 ${icon('chevron')}</button><div class="pk-batch-actions">${btn('批量删除','delete-packs','danger small',state.selectedPacks.size?'':'disabled','trash')}${btn('管理分组','manage-groups','small','','folder')+btn('黑名单','blacklist','small')+btn('合并数据包','merge-packs','small',state.selectedPacks.size<2?'disabled':'')}${btn('导出记录','package-records','small','','clock')}${btn('人工鉴图','inspect-package','small','','image')}${btn('AI 图片、标题检测','review-packages','small','','shield')}</div></div></section>`+
      packageTable(packs);
  }
  const listTime = value => String(value||'—').replace(/(\d{1,2}:\d{2}):\d{2}/, '$1');
  function packageTable(packs) {
    return `<div class="pk-table-wrap"><table class="pk-table pk-packages"><thead><tr><th scope="col" class="col-check"><input id="selectAllPacks" type="checkbox" aria-label="全选数据包" ${packs.length&&packs.every(p=>state.selectedPacks.has(p.id))?'checked':''}></th><th scope="col" class="col-name">数据包名称</th><th scope="col" class="col-group">所属分组</th><th scope="col" class="col-count">商品数</th><th scope="col" class="col-time">时间</th><th scope="col" class="col-actions">操作</th></tr></thead><tbody>${packs.map((p,i)=>{const count=packProducts(p.id).length;return `<tr class="package-row"><td class="col-check"><input type="checkbox" data-pack-select="${e(p.id)}" aria-label="选择数据包 ${e(p.name)}" ${state.selectedPacks.has(p.id)?'checked':''}></td><td class="col-name"><div class="table-identity"><button class="pk-name" data-action="package-open" data-id="${e(p.id)}" title="${e(p.name)}">${e(p.name)}</button><div class="table-meta"><span class="status-pill neutral">${e(p.type)}</span><span>#${String(i+1).padStart(2,'0')}</span></div></div><details class="package-extra"><summary><span class="details-label-desktop">备注</span><span class="details-label-narrow">时间与备注</span></summary><div class="package-extra-body"><div class="package-secondary-times"><span>创建 ${e(p.createdAt)}</span><span>更新 ${e(p.updatedAt)}</span></div><input class="pk-note" data-pack-note="${e(p.id)}" value="${e(p.note)}" placeholder="添加备注" aria-label="数据包备注 ${e(p.name)}"></div></details></td><td class="col-group" data-label="分组"><select aria-label="数据包分组 ${e(p.name)}" data-pack-group="${e(p.id)}">${['未分组',...state.groups].map(g=>`<option ${p.group===g?'selected':''}>${e(g)}</option>`).join('')}</select></td><td class="col-count" data-label="商品数">${count}</td><td class="col-time"><div class="table-time"><span title="${e(p.createdAt)}">创建 ${e(listTime(p.createdAt))}</span><span title="${e(p.updatedAt)}">更新 ${e(listTime(p.updatedAt))}</span></div></td><td class="col-actions"><div class="table-actions package-actions"><button class="pk-open" data-action="package-open" data-id="${e(p.id)}">查看商品 ${icon('arrow')}</button><div class="table-secondary-actions"><button data-action="package-low" data-id="${e(p.id)}">低价检测</button><button data-action="package-export" data-id="${e(p.id)}">${p.type==='图包'?'下载图包':'导出数据包'}</button><button data-action="package-append" data-id="${e(p.id)}">追加数据包</button><button data-action="package-rename" data-id="${e(p.id)}">改名</button></div></div></td></tr>`}).join('')||'<tr><td colspan="6" class="pk-empty">暂无数据包</td></tr>'}</tbody></table></div><div class="pk-pagination"><span>共 ${packs.length} 条</span><select aria-label="每页条数"><option>20 条/页</option></select><button class="icon-button" disabled aria-label="上一页">${icon('left')}</button><span class="pk-page-number">1</span><button class="icon-button" disabled aria-label="下一页">${icon('chevron')}</button></div>`;
  }
  ...
  function tasksPage() {
    ...
    return head('任务中心',btn('清理记录','clear-tasks','','','trash'),`${items.length} 个任务`)+`<div class="tabs">${[['all','全部'],['bulk','批量处理'],['scan','风险检查'],['ai','AI 创作'],['export','导出记录']].map(([key,label])=>`<button class="tab ${state.taskFilter===key?'active':''}" data-action="task-filter" data-value="${key}">${label}</button>`).join('')}</div><div class="task-table"><div class="task-list-head" aria-hidden="true"><span>任务</span><span>进度</span><span>状态</span><span>操作</span></div>${items.length?items.map(t=>`<article class="task-row"><div class="task-icon">${icon(t.kind==='ai'?'sparkle':t.kind==='scan'?'shield':t.kind==='bulk'?'layers':'download')}</div><div class="task-content"><strong title="${e(t.name)}">${e(t.name)}</strong><span>${e(t.time)} · ${t.total} ${t.kind==='ai'?'个候选':'件商品'}${t.kind==='ai'?' · 演示':''}</span></div><div class="task-progress">${t.status==='running'?`<div class="progress-track"><div class="progress-fill" style="width:${t.progress}%"></div></div><span>${t.progress}%</span>`:`<span>${t.status==='done'?'100%':'—'}</span>`}</div><div class="task-state"><span class="status-pill ${t.status==='done'?'success':t.status==='error'?'warning':'neutral'}">${{running:'进行中',done:'已完成',cancelled:'已取消',error:'失败'}[t.status]}</span></div><div class="task-actions">${btn('详情','task-detail','small',`data-id="${t.id}"`)}${t.status==='running'?ib('取消任务','cancel-task','close',`data-id="${t.id}"`):t.kind==='export'&&t.status==='done'?btn('再次下载','task-download','small',`data-id="${t.id}"`):t.kind==='ai'&&t.status==='done'?btn('查看结果','task-result','small',`data-id="${t.id}"`):''}</div></article>`).join(''):'<div class="empty-state"><h2>暂无任务</h2></div>'}</div>`;
  }
  ...
  function inspectionPage() {
    return head('人工鉴图')+`<div class="pk-table-wrap"><table class="pk-table pk-inspections"><thead><tr><th scope="col" class="col-name">数据包</th><th scope="col" class="col-count">商品</th><th scope="col" class="col-count">已鉴图</th><th scope="col" class="col-count">未鉴图</th><th scope="col" class="col-actions">操作</th></tr></thead><tbody>${state.packs.map(p=>{const list=packProducts(p.id).filter(p=>!excluded(p)),done=list.filter(p=>{const paths=Object.keys(roles).filter(k=>k!=='video').flatMap(k=>p[k]||[]);return paths.length&&Array.isArray(p.reviewedImages)&&paths.every(path=>p.reviewedImages.includes(path));}).length;return `<tr><td class="col-name"><div class="table-identity"><strong class="table-title" title="${e(p.name)}">${e(p.name)}</strong><span class="mobile-summary">${list.length} 件商品 · 已鉴 ${done} · 未鉴 ${list.length-done}</span></div></td><td class="col-count">${list.length}</td><td class="col-count">${done}</td><td class="col-count">${list.length-done}</td><td class="col-actions"><div class="table-actions">${btn('开始鉴图','package-inspect','primary small',`data-id="${p.id}"`)}</div></td></tr>`}).join('')||'<tr><td colspan="5" class="pk-empty">暂无数据包</td></tr>'}</tbody></table></div>`;
  }
  function wordsPage() {
    const types={title:'标题违规词',brand:'品牌关注词',image:'图片违规词'};
    return head('违规词库',btn('导入词库','word-import','primary','','upload'))+`<div class="tabs">${[['all','全部'],...Object.entries(types)].map(([key,label])=>`<button class="tab ${state.wordType===key?'active':''}" data-action="word-type" data-value="${key}">${label}</button>`).join('')}</div><div class="pk-table-wrap"><table class="pk-table pk-words"><thead><tr><th scope="col" class="col-name">名称</th><th scope="col" class="col-type">类型</th><th scope="col" class="col-count">词数</th><th scope="col" class="col-actions">操作</th></tr></thead><tbody>${state.wordLibraries.filter(w=>state.wordType==='all'||state.wordType===w.type).map(w=>`<tr><td class="col-name"><div class="table-identity"><strong class="table-title" title="${e(w.name)}">${e(w.name)}</strong><span class="mobile-summary">${types[w.type]} · ${w.words.length} 词</span></div></td><td class="col-type">${types[w.type]}</td><td class="col-count">${w.words.length}</td><td class="col-actions"><div class="table-actions">${btn('编辑','word-edit','small',`data-id="${w.id}"`)}${btn('删除','word-delete','danger small',`data-id="${w.id}"`)}</div></td></tr>`).join('')||'<tr><td colspan="4" class="pk-empty">暂无词库</td></tr>'}</tbody></table></div>`;
  }
  ...
  function mediaPage() {
    const items=mediaItems().filter(i=>state.mediaType==='all'||i.kind===state.mediaType);
    return head('媒体资源',`<label class="button primary">${icon('upload')}上传资源<input id="mediaUpload" type="file" accept="image/png,image/jpeg,image/webp,video/mp4" multiple hidden></label>`,`${items.length} 个素材`)+`<div class="tabs">${[['all','全部'],['image','图片'],['video','视频']].map(([k,v])=>`<button class="tab ${state.mediaType===k?'active':''}" data-action="media-type" data-value="${k}">${v}</button>`).join('')}</div><div class="v3-media-grid">${items.map((m,i)=>`<article class="media-card">${m.kind==='video'?`<div class="media-preview"><video src="${e(media(m.url))}" controls preload="metadata"></video></div>`:`<button class="media-preview" data-action="zoom" data-src="${e(src(m.url))}" aria-label="预览 ${e(m.name)}"><img src="${src(m.url)}" alt="${e(m.name)}" loading="lazy"></button>`}<div class="media-body"><strong title="${e(m.name)}">${e(m.name)}</strong><span>${m.uploaded?'素材库':e(m.label)} · ${m.uses} 处引用</span></div><div class="media-actions">${m.kind==='image'?btn('使用素材','media-use','small',`data-index="${i}"`):''}${m.uploaded?btn('移除','media-remove','small danger',`data-id="${e(m.id)}"`):''}</div></article>`).join('')}</div>`;
  }
